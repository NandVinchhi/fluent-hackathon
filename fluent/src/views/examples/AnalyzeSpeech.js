...
function AnalyzeSpeech() {
  ...
  const handleProcess = () => {
    console.log(window.sessionStorage.getItem("input_audio1"))
    setIsLoading(true);
    fetch('http://3.131.38.145:8080/audio', {method: 'post',headers: {'Accept': 'application/json','Content-Type': 'application/json'},
                          body: JSON.stringify({userid:window.sessionStorage.getItem("id"), audio:window.sessionStorage.getItem("input_audio1")})}).then((Response) => Response.json()).then((Result) => {
  ...
  React.useEffect(() => {
    ...
    window.sessionStorage.removeItem("input_audio1");
    window.scrollTo(0, 0);
                ...
                <Row>
                  <Col className="mr-auto ml-auto" md="8">
                    ...
                    <h4 className="description">
                      Get analytics on realtime audio and video data.
                    </h4>
                  </Col>
                </Row>
              </Container>  
